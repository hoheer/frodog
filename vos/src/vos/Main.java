	      ...
	      File f = new File("oauth.json");
	        ...
	        if (f.exists()) {	//기존에 발급받은 토큰이 존재하는 경우       	
	            ...
	            try {
	                ...
	                boolean expired = now > auth.getExpires(); //토큰 내 만기일과 현제 시간값 비교 
            ...
            if(request_new) {
                ...
                try {
                  ...
                  //token.json작성을 위해 token객체인 rsp를 json으로 변환  
...
public static String get_player_uid_header (String header) {
    ...
    if (player_uid == null) { //player_uid 값이 존재하지 않는 경우 새로 발급 받는다
